...
const SignIn = (props) => {
    ...
    const onSignInHandler = async () => {
        try {
            let user = await userService.login(userEmail, userPassword);
            props.login(user);
...
const mapStateToProps = (state) => {
    return {
        ...
        selected: state.subject.selected
    }
}

const mapDispatchToProps = dispatch => {
    return {
        selectSubject: (code) => dispatch(subjectActions.set(code)),
        setErrorAlert: (message) => dispatch(alertActions.error(message)),
        setWarningAlert: (message) => dispatch(alertActions.warning(message)),
